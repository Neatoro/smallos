...
ASM_FILES := $(shell find -name "*.s" -path "./kernel/arch/$(TARGET)/*" | sed -e 's/\.s/.o/')
BOOT_FILES := $(shell find -name "*.s" -path "./boot/$(TARGET)/*" | sed -e 's/\.s/.o/')
...
compile: $(KERNEL_DEFAULT_FILES) $(ARCH_FILES) $(LIB_FILES) $(ASM_FILES) $(BOOT_FILES)
